...
int main(int argc, const char* argv[]) {
    WOODPECKER_INIT;
  //  FixedSizePool<std::string, 1> q;
   // std::cout << *q.construct(std::string("FixedSizePool string-el is működik\n"));
   // DynamicPool<std::string, 1> w;
   // std::cout << *w.construct(std::string("DynamicPool string-el is működik\n"));
    long* p = nullptr;
    ...
    TEST("Dynamic construct", 4) {
        ...
        CHECK_NOEXC(d.allocated.push_back(d.a.construct(11)));
        std::sort(d.allocated.begin(), d.allocated.end());
        CHECK_EQ((std::abs(d.allocated.front() - p) > 10), true);
        CHECK_EQ(d.sum(), 56);
    ...
    //std::vector<std::string, DynamicPool<std::string, 10>>vstring{"a", "b", "c"};
    //std::cout << vstring[0] << vstring[1] << vstring[2] << std::endl;
    //std::list<std::string, DynamicPool<std::string>> l;
    //l.push_back(std::string("kecske\n"));
    //std::cout << l.front();
